    ...
    parfor k = 1:K2
        ...
        DiffCoef = @(x,y) ( exp( sum (sin( 2*pi*(x*sin(angle_coef1) + y*cos(angle_coef1))./epsilon1 ) .* xi(1:6)/1.5 ) )  )...,
                        ...
                        + ( exp( sum (sin( 2*pi*(x*sin(angle_coef2) + y*cos(angle_coef2))./epsilon2 ) .* xi(7:12)/1.5 ) )  )...,
                            * any((x>=rectK0(:,1)) .* (x<=rectK0(:,1)+rectK0(:,3)) .* (y>=rectK0(:,2)) .* (y<=rectK0(:,2)+rectK0(:,4)) );
        A1 = StiffMat2D_sparse(DiffCoef,p,t);
        Flux1 = FluxMat2D_sparse(DiffCoef,p,t);
        %tic;
        u1 = (A1 + R1) \ (b1 + r1);
        ...
        u1_Fluxproj = Phi * ((Phi' * Flux1 * Phi) \ (Phi' * Flux1 * u1));
        %tic;
        ...
        Fluxerr(k) = sqrt( ((u1_POD-u1)'*Flux1*(u1_POD-u1)) / (u1'*Flux1*u1) );
        ...
        Fluxproj(k) =  sqrt( ((u1_Fluxproj-u1)'*Flux1*(u1_Fluxproj-u1)) / (u1'*Flux1*u1) );
    end;
...
save('Example5_12rvs_global_data','U','M1','H1','-v7.3');
save('Example5_12rvs_global_Galerkin','EffR','accweight','Fluxerr_mean','Fluxerr_std','Fluxproj_mean','Fluxproj_std','L2err_mean','H1err_mean','Engerr_mean','L2err_std','H1err_std','Engerr_std','L2proj_mean','H1proj_mean','Engproj_mean','L2proj_std','H1proj_std','Engproj_std','K2','-v7.3');
...
save('Example5_12rvs_local_data','loc_idx','M1_loc','H1_loc','-v7.3');
save('Example5_12rvs_local_Taylorexpansion','EffR','accweight','Fluxerr_mean','Fluxerr_std','Fluxproj_mean','Fluxproj_std','L2err_mean','H1err_mean','Engerr_mean','L2err_std','H1err_std','L2proj_mean','H1proj_mean','Engproj_mean','Engerr_std','L2proj_std','H1proj_std','Engproj_std','K2','-v7.3');
